...
def parser(data, comments):
    ...
    for cm in comment:
        ...
        # in case the comment user name filed is empty
        if name == "":
            name = "anonymous"
        ...
        # Adding the data retrieved from each comment to the file where we keep track of the video field metadata
        coms.append(com)
...
def get_comments(posts):
    ...
    for post_url in posts:
        print(post_url)
        post_id = post_url.split("/")[-1]  # getting the post id from the post url list created
